...
#define HEIGHT 800
	...
	//起始点定义
int start_point_x= 409;//在这改动图像和二维数组的关系
int start_point_y= 399 ;
//目标点定义
...
void *findPathAstar_task(void *)
{
  ...
  while(1){
	...
	//waitKey(0);
	//图像阈值化操作
	threshold(src, grayMaskSmallThresh, 100, 255, CV_THRESH_BINARY_INV);//反相选择 >230 =0,else= 1 ，二值化的地图中，0代表可以到达。
    //获取 mat 的行和列
